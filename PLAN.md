...
## Phase 10: Biome Progression — Phase 1 ✅

### 10A. Biome Data Table ✅
- [x] `BIOMES` constant: 5 entries (Classic Void, Rubble Field, Drift Expanse, Dense Belt, Mire Wastes)
- [x] Each biome: `{ id, name, scoreMult, friction, countMult, speedMult, safeZoneMult, hpBonus, powerupLuckBonus, starBitMult, rotMult, magnetMult, enemyType, specialObject, specialChance, desc, palette }`
- [x] `getActiveBiome()` helper (null-safe, defaults to Classic Void)

### 10B. Engine Integration ✅
- [x] `getLevelSpeedMult` reads `biome.speedMult`
- [x] `getLevelAsteroidCount` reads `biome.countMult`
- [x] `getLevelSafeZone` reads `biome.safeZoneMult`
- [x] `getLevelHealthBonus` adds `biome.hpBonus`
- [x] `rollPowerup` adds `biome.powerupLuckBonus`
- [x] `spawnStarBits` applies `biome.starBitMult` (via star bit amount)
- [x] `updateShip` uses `biome.friction`
- [x] `updateAsteroids` applies `biome.rotMult` to rotation
- [x] `getMagnetRadius` applies `biome.magnetMult`

### 10C. Constellation Map ✅
- [x] New `constellation_map` game state
- [x] Hybrid canvas+DOM: canvas starfield background + DOM clickable nodes
- [x] Each node: biome name, score mult, best score, lock/equip status
- [x] Keyboard + touch navigation
- [x] Main menu entry: `[C] Constellation` (replaces direct Start Game → modifier flow)
- [x] Flow: `menu → constellation_map → modifier_select → playing`

### 10D. Profile + Unlock Logic ✅
- [x] New profile fields: `unlockedBiomes`, `selectedBiome`, `biomeBestScores`
- [x] Default: only `classic_void` unlocked
- [x] Unlock chain: clearing Level 20 in biome N unlocks biome N+1
- [x] Per-biome best score tracked in profile

### 10E. Palette System ✅
- [x] CSS variables: `--biome-accent`, `--biome-bg-tint`
- [x] Applied to HUD accent color + background star tint
- [x] No gameplay effect, just visual identity

### 10F. Biome Enemies ✅
- [x] Drift Mines (Rubble Field): pulsing mines, explode on proximity, chain-react
- [x] Comets (Drift Expanse): fast screen-crossers, drop star bits
- [x] Turrets (Dense Belt): stationary, periodic shots at ship
- [x] Swarmers (Mire Wastes): tiny, slow chase, spawn in groups
- [x] Enemy bullets vs ship collision
- [x] Bullet vs enemy collision
